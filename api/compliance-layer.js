...
function patchHeroMessaging(out) {
  return String(out || '')
    ...
    .replace(/<div class="hero-eyebrow label">Consultant Expert — Martinique<\/div>/g, '<div class="hero-eyebrow label">Commande publique — Martinique & Antilles</div>')
    .replace(/<h1 class="display hero-title">[\s\S]*?<\/h1>\s*<p class="lead hero-subtitle">[\s\S]*?<\/p>/i, '<h1 class="display hero-title">Entreprises et entités publiques :<br><em>structurer la commande publique avec méthode.</em></h1><p class="lead hero-subtitle">J’accompagne les entreprises dans la structuration de leurs réponses aux appels d’offres, et les entités publiques dans la préparation de procédures d’achat plus claires, cohérentes et maîtrisées.</p>')
    ...
    .replace(/J'aide à construire le meilleur dossier possible, conforme aux règles et aligné sur les attentes de l'acheteur\./g, 'J’aide à structurer des dossiers plus lisibles et des procédures plus robustes, dans le respect des règles applicables et d’un cadre déontologique strict.');
}
